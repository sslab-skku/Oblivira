...
/* untrusted config */
#define NUM_DID_REQ_THR 16
#define NUM_DOC_FETCH_THR 32
...
void *did_req_handler(void *arg) {
  ...
  if (connect(universalfd, (struct sockaddr *)&clientaddr, sizeof(clientaddr)) <
      0) {
...
#if defined(OBLIVIRA_PRINT_LOG)
  std::cout
      << "[UNTRUSTED][did_req_handler] request to universal resolver succeded!"
      << std::endl;
#endif
...
void *did_doc_fetch_handler(void *arg) {
  ...
  if (sgxStatus != SGX_SUCCESS) {
    std::cout
        << "[Untrusted][ecall_request_blockchain] Failed to request blockchain!"
        << std::endl;
