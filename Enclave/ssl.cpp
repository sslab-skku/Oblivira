...
int enc_wolfSSL_connect(long sslId)
{
  ...
  // obvenc_err("Calling wolfssl_connect\n");
...
long ecall_init_ctx_server(void)
{
  ...
  if (ctx == NULL)
  {
    obvenc_err("[ENCLAVE][init_ctx_server] Failed to create new context!\n");
    return -1;
  }
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_server] wolfSSL_CTX_use_certificate_buffer "
               "failure\n");
    return -1;
  }
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_server] wolfSSL_CTX_use_PrivateKey_buffer "
               "failure\n");
    return -1;
  }
...
long ecall_init_ctx_client(void)
{
  ...
  if (ctx == NULL)
  {
    obvenc_err("[ENCLAVE][init_ctx_client] Failed to create new context!\n");
    return -1;
  }
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_client] wolfSSL_CTX_set_cipher_list failure\n");
    return -1;
  }
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_client] "
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_client] wolfSSL_CTX_use_PrivateKey_buffer "
               "failure\n");
    return -1;
  }
  ...
  if (ret != SSL_SUCCESS)
  {
    obvenc_err("[ENCLAVE][init_ctx_client] Error loading cert\n");
    return -1;
  }
